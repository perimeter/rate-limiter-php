...
namespace Perimeter\RateLimiter\Storage;
...
class DoctrineMeterStorage implements MeterStorageInterface, MeterStorageAdminInterface
{
    ...
    {
        $query = $this->_em->createQuery('SELECT m FROM Perimeter\RateLimiter\Entity\RateLimitMeter m WHERE m.meter_id = ?1');
    ...
    public function findBySearch($search, $limit = 10, $page = 1)
    {
        ...
        $query = $this->_em->createQuery('SELECT m FROM Perimeter\RateLimiter\Entity\RateLimitMeter m WHERE m.meter_id like ?1');
